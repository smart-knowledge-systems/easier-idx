...
export function kmeans(
  ...
  if (k > items.length)
    throw new Error(`k (${k}) exceeds item count (${items.length})`);
...
function sampledSilhouette(
  ...
  for (const i of sampleIndices) {
    ...
    if (myCluster.length <= 1) {
      ...
      let count = 0;
      const step =
        myCluster.length <= MAX_INTRA ? 1 : myCluster.length / MAX_INTRA;
    ...
    for (let c = 0; c < k; c++) {
      ...
      const step =
        otherCluster.length <= MAX_INTRA ? 1 : otherCluster.length / MAX_INTRA;
      for (let s = 0; s < limit; s++) {
