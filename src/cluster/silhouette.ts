...
export function silhouetteScore(
  ...
  for (const i of sampleIndices) {
    ...
    if (myCluster.length > 1) {
      ...
      const step = myCluster.length <= maxIntraCluster ? 1 : myCluster.length / maxIntraCluster;
    ...
    for (const [cid, indices] of clusterIndex) {
      ...
      const step = indices.length <= maxIntraCluster ? 1 : indices.length / maxIntraCluster;
      for (let s = 0; s < limit; s++) {
